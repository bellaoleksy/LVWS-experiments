...
expchem <- expchem %>%
  mutate(changeDOC_mgL=DOC_mgL-initial_DOC_mgL,
...
expchem_trim <- expchem %>%
  select(sample_id, temperature, treatment, tempxtrt, rep, chl_a_phyto,changeDOC_mgL, changePO4_uM, changeTDN_uM, changePO4_uM, changeNH4_mgL, percTDNuptake, percPO4uptake,TDNuptake_dailyrate, DOC_mgL, NH4_mgL)
...
```{r histograms of each group NH4, echo=FALSE, message=FALSE, warning=FALSE}
library(DescTools)

Desc(NH4_mgL ~ temperature, data=expchem, digits=1, plotit=TRUE)
Desc(NH4_mgL ~ treatment, data=expchem, digits=1, plotit=TRUE)

library(lattice)

histogram(~ NH4_mgL | temperature, 
          data=expchem,
          layout=c(1,3),
          col="#3166A9",
          main="NH4 at end of experiment by Temperature")
histogram(~ NH4_mgL | treatment, 
          data=expchem,
          layout=c(1,4),
          col="#3166A9",
          main="NH4 at end of experiment by Nutrient Treatment")

boxplot(NH4_mgL ~ temperature * treatment, data=expchem)
# ggsave("boxplot.png")

```

```{r NH4 data visualization}
# Change stripchart colors by groups
...
```{r NH4 ANOVA Type II, echo=FALSE}
NH4_Model1 <- lm(NH4_mgL ~  temperature * treatment -1 , data=expchem) #No intercept model 
library(emmeans) #Estimated Marginal Means
...
```{r NH4 qqplots and residuals, echo=FALSE}
summary(NH4_Model1)
qqnorm(NH4_Model1$res)
plot(NH4_Model1$fitted,NH4_Model1$res,xlab="Fitted",ylab="Residuals")
leveneTest(NH4_mgL ~ treatment * temperature,data=expchem)

```

We don't exactly meet the assumptions of normality. 

```{r NH4 model log transformed, echo=FALSE}
expchem <- data.frame(expchem, logNH4 = log10(expchem$NH4_mgL))

NH4_Model2 <-lm(logNH4 ~  temperature * treatment -1 , data=expchem) #No intercept model 
summary(NH4_Model2)
...
leveneTest(logNH4 ~ treatment * temperature,data=expchem)
...
Pairwise comparsions for each level of temperature
```{r NH4 pairwise trt x temp , echo=FALSE}
library(emmeans) #Estimated Marginal Means
emms_NH4_trtxtemp <- emmeans(NH4_Model1, pairwise ~ treatment | temperature)
#Pairwise comparsions for each level of temperature
emms_NH4_trtxtemp
```

Pairwise comparsions for each level of nutrient treatment
```{r NH4 pairwise temp x trt, echo=FALSE}
emms_NH4_tempxtrt <- emmeans(NH4_Model1, pairwise ~ temperature | treatment)
#Pairwise comparsions for each level of nutrient treatment
emms_NH4_tempxtrt

# cld(emms_TDN_tempxtrt) # implicitly uses by "temperature"

cld(emms_NH4_tempxtrt, by=NULL, Letters= "||||||||", alpha=0.01)
cld(emms_NH4_tempxtrt)
```
...
boxplot(changeDOC_mgL ~ temperature * treatment, data=expchem_trim)
...
#Bar plot of net DOC release 
expcolors <- c("#40A339", "#3166A9", "#DE0019", "#82388E")
netDOCrelease <- ggplot(expchem_changesummary, aes(x=treatment, y=meanchangeDOC_mgL, fill=treatment)) + theme_bw()+
   geom_bar(color="black",stat="identity", position="dodge") +
  scale_fill_manual(values=expcolors,
                    name="Nutrient\nTreatment",
                    labels=c("Control", "Nitrogen", "Phosphorus", "Both (N & P)"))+
...
compare_means(changeDOC_mgL ~ treatment ,  data = expchem_trim, group.by="temperature")
DOC_comparisons <- list(c("C","P"),c("C","NP"), c("P", "NP"),c("N","P"))
DOCrelease_boxplot <- ggplot(expchem_trim, aes(x=temperature, y=changeDOC_mgL, fill=treatment)) +
  geom_boxplot()+
  stat_boxplot(geom ='errorbar') +
  scale_fill_manual(values=trtcolors,
                    name="Nutrient\nTreatment",
...
```{r histograms of each group PO4 2, echo=FALSE, message=FALSE, warning=FALSE}
library(DescTools)
expchem$temperature <- as.factor(expchem$temperature)
...
histogram(~ changePO4_uM | treatment, 
          ...
          main="Change in PO4 by Nutrient Treatment")
# Change stripchart colors by groups
ggplot(expchem, aes(x=temperature, y=PO4_uM, color=treatment)) +
   geom_boxplot(position=position_dodge(0.8))+
    geom_jitter(position=position_jitter(0.2))+
  scale_color_manual(values=c("#40A339", "#3166A9", "#DE0019", "#82388E"))+
  theme_classic2(base_size=16)
...
```{r GPP pairwise trt x temp 2, echo=FALSE}
library(emmeans) #Estimated Marginal Means
